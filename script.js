...
async function startRecording() {
    ...
    recorder.ondataavailable = async function (event) {
        ...
        try {
            const response = await fetch('https://geoglide.org/api/encode-video.js', {
                method: 'POST',
                body: formData,
            });
...
console.error('Map source "point" not found.');
    ...
    document.getElementById('replay').disabled = true; // Disable 'replay' button during animation
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            if (routes && routes.length > 0) {
                // Check if the route crosses international waters (assume if it's a long route, it may cross)
                const routeDistance = routes[0].distance;
                const maxLandDistance = 100000; // Adjust this threshold as needed
                resolve(routeDistance <= maxLandDistance);
            } else {
                resolve(false);
            }
        })
...
const newStyle = currentStyle.name === 'Satellite Streets' ? 'mapbox://styles/mapbox/dark-v10' : 'mapbox://styles/mapbox/satellite-streets-v12';
loadMap(newStyle);
});
...
document.getElementById('distanceCheckbox').addEventListener('change', () => {
toggleDistanceVisibility();
});

document.getElementById('animationSpeedSlider').addEventListener('input', () => {
const speedValue = document.getElementById('animationSpeedSlider').value;
